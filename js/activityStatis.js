...
$(function(){
    ...
    // let activityid_0 = $("#selactivity").val();
    readTextFile("../config.json",function(text){
        let data = JSON.parse(text);
        IPconf = data.IPconf;
        prot = data.prot;
        httpHead = data.httpHead;
        //timelengthStatis(GetCookies("token"),pf_0,sid_0,ctype_0,begintime_0,endtime_0,activityid_0);
    });
    tablebody(2);
    ...
    document.getElementById('searchbtn').onclick = function(){
        ...
        let activityid = $("#selactivity").val();
        if(activityid == "all"){
            alert("请选择活动");
            return;
        }
        timelengthStatis(GetCookies("token"),pf,sid,ctype,begintime,endtime,activityid);
    }
})

function timelengthStatis(CS_token,CS_pf,CS_sid,CS_ctype,CS_beginDate,CS_endDate,activityid){
    if(!GetCookies("token")){
        // alert("账号已过期，请重新登陆");
        window.location.href = "login.html";
        return;
    ...
    indexid = 17 + Number(activityid);
    let url = "/statistics" + indexid;
    httpRequest.open('POST', httpHead+ IPconf + ':' + prot + url,true);
